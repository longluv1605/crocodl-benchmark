...
from lamar.tasks import (
    FeatureExtraction, PairSelection, FeatureMatching, Mapping, PoseEstimation, ChunkAlignment, CrossValidation)
...
def run(outputs: Path,
    ...
    configs = {
        ...
        'cross_valid': CrossValidation.methods['rig' if is_rig and do_rig else 'single_image'],
    ...
    if is_sequential:
        ...
        cross_valid = CrossValidation(
            ...
            (R_threshold, t_threshold), top, r_margin
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--top', type=int, default=10, required=True)
    parser.add_argument('--r_margin', type=float, default=10.0, required=True)
    ...
    if isinstance(results_, str):
        logger.info('Cross validated poses is saved to %s', results_)
    else:
        logger.info('Results:\n%s', pformat(results_))