...
CAPTURE_DIR=/home/robotics/VisualLocalization/capture
...
LOCATION=("ARCHE_D2")
OUTPUT_DIR="trinh/benchmarking_results"
...
R_THRESHOLD=20.0
T_THRESHOLD=20.0
MODERATE_R_THRESHOLD=5.0
SEVERE_R_THRESHOLD=15.0
TOP=3
...
for ref in "${DEVICES_REF[@]}"; do
  for query in "${DEVICES_QUERY[@]}"; do
    ...
    python -m lamar.cross_valid \
      ...
      --moderate_R_threshold "$MODERATE_R_THRESHOLD" \
      --severe_R_threshold "$SEVERE_R_THRESHOLD" \
      --top "$TOP" \
      $is_rig_flag
